...
mkdir -p "$campaign_dir/raw" "$campaign_dir/records" "$campaign_dir/slurm" "$campaign_dir/sampling" "$campaign_dir/tracing"

ft3_export_python_runtime 3.15t
runner_python="$(ft3_venv_python 3.15t)"
"$runner_python" "$SCRIPT_DIR/make_py315_profile_matrix.py" --profile "$profile" --output "$manifest"
...

{
    ...
    printf 'time_limit = %s\n' "$time_limit"
    printf 'run_count = %s\n' "$run_count"
    printf 'affinity_note = OMP_PROC_BIND, OMP_PLACES and GOMP_CPU_AFFINITY disabled by py315_profile_one\n'
...
/bin/sbatch "${sbatch_args[@]}" "$SCRIPT_DIR/py315_profile_array_run.sbatch"
