...
profile="${1:-profile_versions_core}"
if [ -n "${PARTITION:-}" ]; then
    partition="$PARTITION"
else
    partition="medium"
...
mkdir -p "$campaign_dir/raw" "$campaign_dir/records" "$campaign_dir/slurm" "$campaign_dir/cache" "$campaign_dir/perf"
...
/bin/sbatch "${sbatch_args[@]}" "$SCRIPT_DIR/profile_array_run.sbatch"
