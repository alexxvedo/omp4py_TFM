...
La restricción no impide toda forma de paralelismo, porque Python puede delegar
cómputo a extensiones nativas o distribuir trabajo entre procesos. Sin embargo,
esas alternativas cambian el modelo de programación. El punto relevante para
este trabajo es el paralelismo de memoria compartida expresado directamente en
código Python, con hilos y directivas de estilo OpenMP. La relación con otros
enfoques se resume en la Sección~\ref{sec:trabajo-relacionado}.
...
Python, y es la línea base funcional. El modo~1 usa el runtime por
...
Esta diversidad de modos permite aislar tres factores de rendimiento, el coste del
...
GIL en lugar de eliminarlo. Algunas delegan el cómputo a bibliotecas nativas,
otras compilan subconjuntos del lenguaje y otras distribuyen el trabajo entre
procesos. La Tabla~\ref{tab:trabajo-relacionado-python} resume la posición de
OMP4Py frente a esas alternativas.
...
\hline
\textbf{Enfoque} & \textbf{Modelo de programación} & \textbf{Relación con este TFM} \\
\hline
NumPy~\cite{NumPy2020} & Operaciones vectoriales ejecutadas en código nativo,
habitualmente con liberación del GIL. & Muy eficiente cuando el algoritmo encaja
en primitivas vectoriales, pero el paralelismo no se expresa como regiones,
bucles y sincronización de memoria compartida al nivel del programa Python. \\
\hline
Cython~\cite{Cython2011} y Numba~\cite{Numba2015} & Compilación de funciones
numéricas a código nativo, con restricciones diferentes sobre tipos y
constructos soportados. & Reducen el coste del intérprete. En OMP4Py, Cython es
precisamente la capa que permite estudiar cuánto aporta el tipado frente al
paralelismo entre hilos. \\
\hline
PyOMP~\cite{PyOMP2021} & Directivas OpenMP sobre funciones compiladas con
Numba. & Es la alternativa más próxima por usar directivas, pero queda ligada al
subconjunto compilable por Numba. OMP4Py parte de código Python con directivas
propias y compila opcionalmente los kernels mediante Cython. \\
\hline
\texttt{multiprocessing}, mpi4py~\cite{mpi4py2011} y Dask~\cite{Dask2015} &
Paralelismo mediante procesos y, en mpi4py/Dask, posible distribución entre
nodos. & Escalan más allá de un proceso, pero cambian el modelo a memoria
privada y comunicación explícita o gestionada. Este TFM se centra en memoria
compartida y equipos de hilos al estilo OpenMP. \\
\hline
OMP4Py~\cite{PineiroOMP4PyFGCS2026,PineiroOMP4PyCGO2026} & Directivas de estilo
OpenMP integradas en código Python, con runtime de hilos y modos interpretados o
compilados. & Permite evaluar, sobre el mismo código fuente, el coste del runtime
Python, el efecto del tipado Cython y la escalabilidad real en Python
\emph{free-threaded}. \\
\hline
\end{tabular}
}
\caption{Relación entre OMP4Py y otros enfoques de paralelismo o aceleración en
Python.}
\label{tab:trabajo-relacionado-python}
\end{table}

La eliminación del GIL en las versiones \emph{free-threaded}~\cite{PEP703} abre
una vía distinta, la de ejecutar hilos Python reales sobre memoria compartida. OMP4Py
se sitúa en esa vía al trasladar el modelo de directivas de
OpenMP~\cite{OpenMPSpec,DagumMenon1998} al código Python. A diferencia de los
trabajos que presentan la propia biblioteca, esta memoria evalúa su
comportamiento sobre aplicaciones completas, con verificación NAS, cuatro modos
de ejecución y varias versiones del intérprete.
...
de los NPB a Python a partir de las versiones secuenciales en C++~\cite{NPBPython},
que sirven de base secuencial para la adaptación descrita en el
Capítulo~\ref{chap:adaptacion-nas}. Hasta donde alcanza este trabajo, la
evaluación de OMP4Py sobre kernels NPB completos, con comparación sistemática de
modos y de versiones \emph{free-threaded}, no se había abordado.
